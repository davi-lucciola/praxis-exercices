...
import type { ToolStatus } from '@/lib/blocks'
...
export function WeatherCard({ city, status, temperature, condition }: WeatherCardProps) {
  return (
    <article
      aria-busy={status === 'running'}
      aria-label={`Briefing de ${city || 'estação'}`}
      className={cn(
    ...
    >
      <div className="flex items-center justify-between bg-accent px-4 py-2 text-accent-foreground">
        ...
        <p className="font-mono text-xs tracking-widest uppercase">{city || '—'}</p>
      </div>
